defmodule TwitterApiClient.API.Users do
  @moduledoc """
  ...
  def user_lookup(id_or_name_list, options) when is_list(id_or_name_list) do
    params = [
      ...
      |> TwitterApiClient.Parser.parse_batch_user_lookup_params
  ...
  def user_profile_banner(user_id) do
    ...
    |> TwitterApiClient.Parser.parse_profile_banner
  end
  ...
  def user(user_id, options) when is_list(options) do
    params = TwitterApiClient.Parser.parse_request_params(parse_user_id_param(user_id) ++ options)
  ...
  def user(user_id, screen_name, options \\ []) do
    params = TwitterApiClient.Parser.parse_request_params([user_id: user_id, screen_name: screen_name] ++ options)
    request(:get, "1.1/users/show.json", params)
    |> TwitterApiClient.Parser.parse_user
  end
end
