defmodule TwitterApiClient.API.Streaming do
  @moduledoc """
  ...
  defp spawn_async_request(req=%AsyncRequest{}) do
    oauth = TwitterApiClient.Config.get_tuples |> TwitterApiClient.API.Base.verify_params
    spawn(fn ->
      response = TwitterApiClient.OAuth.request_async(
  ...
  def parse_tweet_message(json, configs) do
    try do
      case TwitterApiClient.JSON.decode(json) do
        {:ok, tweet} ->
          case parse_message_type(tweet,configs) do
            {:msg, _}       -> {:stream, TwitterApiClient.Parser.parse_tweet(tweet)}
  ...
  defp parse_control_message(message) do
    case message do
      %{:delete => tweet} ->
        {:stream, %TwitterApiClient.Model.DeletedTweet{status: tweet.status}}

      %{:limit => limit} ->
        {:stream, %TwitterApiClient.Model.Limit{track: limit.track}}

      %{:warning => warning} ->
        {:stream, %TwitterApiClient.Model.StallWarning{
